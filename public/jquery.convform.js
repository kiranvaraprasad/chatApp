...
ConvState.prototype.printQuestion = function(){
    ...
    console.log(this.current.input.type);
        
    if(this.current.input.type=='text'){
        setTimeout(function(){$('#userInput,button.submit').show();},1000)
    }
    else{
        setTimeout(function(){$('#userInput,button.submit').hide();},1000)
    }
    ...
    setTimeout(function(){
        ...
        if(this.current.input.type=="select"){
            this.printAnswers(this.current.input.answers, this.current.input.multiple, questions);
        }
        this.scrollDown();
...
ConvState.prototype.printAnswers = function(answers, multiple, questions){
    if(answers[0].text == 'Absolutely'){
        $.ajax({
            method:"POST",
            ...
            if(answers.hasOwnProperty(i)){
                var option = $('<div class="option">'+answers[i].text+'</div>')
                    ...
                    .click(function(event){
                        ...
                        responseArray.push({question:questions[0],answer:this.current.input.selected})
        ...
        if(inputs.length) {
            ...
            switch(parameters.typeInputUi) {
                case 'input':
                    ...
                    $(parameters.inputIdHashTagName).hide();
                    break;
                case 'textarea':
            ...
            $(inputForm).find(parameters.inputIdHashTagName).keypress(function(e){
                if(e.which == 13) {
                    ...
                    e.preventDefault();
                     ...
                     responseArray.push({question:state.current.input.questions[0],answer:input});
                    if(state.current.input.type=="select" && !state.current.input.multiple){
